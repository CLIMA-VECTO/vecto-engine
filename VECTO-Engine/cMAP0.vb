...
lbEr:
	...
	Public Function Init() As Boolean
		...
		For Each mp0 In MapPoints
			...
			If Not Matched Then
                ...
                WorkerMsg(tMsgID.Err, "  >>> Calculated target rpm values are: " & String.Join(", ", TargetRPMs.ToArray) & " [1/min].")
				Return False
			End If
			...
			If rl.MapPoints.Count > 2 Then
				...
				For i = 1 To rl.MapPoints.Count - 2	'skip highest torque 
                    ...
                    If TqStepAvg > TqStepMin + 2 * TqStepTol Then
                        WorkerMsg(tMsgID.Err, "Invalid torque interval in FC map at " & rl.TargetRPM & " [1/min] between " & rl.MapPoints(i - 1).Tq & "[Nm] and " & rl.MapPoints(i).Tq & "[Nm]!")
                        WorkerMsg(tMsgID.Err, "	 Calculated torque interval = " & TqStepAvg & " [Nm].")
                        WorkerMsg(tMsgID.Err, "	 Maximum allowed torque interval = " & TqStepMin & " [Nm] + " & 2 * TqStepTol & "[Nm] tolerance.")
                        Return False
                    End If
                Next
    ...
    Public Function LimitR49toMap() As Boolean
        ...
        For i = 0 To R49.iDim
            ...
            If Tq > TqMaxMap Then
                R49.LTq(i) = TqMaxMap
                WorkerMsg(tMsgID.Warn, "   Parent R49 Full Load torque exceeds Fuel Map max torque at " & Math.Round(nU, 2) & " [1/min] by " & Math.Round(Tq - TqMaxMap, 2) & "[Nm] !")
                WorkerMsg(tMsgID.Warn, "     >>> Maximum torque at " & Math.Round(nU, 2) & " [1/min] will be limited to " & Math.Round(TqMaxMap, 2) & "[Nm] for WHTC simulation!")
            End If
        Next
        ...
        For Each rl In RPMlists.Values
            ...
            For Each mp1 In rl.MapPoints
                ...
                Else
                    ...
                    If mp1.Tq - TqStepTol > Tq Or Math.Abs(mp1.Tq - Tq) < TqStepTol Then
                        ...
                        If i < 1 Then
                            WorkerMsg(tMsgID.Err, "Extrapolation not possible: Only one map point below R85 full load at " & rl.TargetRPM & " [1/min] !")
                            Return False
                        End If
            ...
            Else
                ...
                If Math.Abs(Tq - rl.MapPoints.Last.Tq) > TqDistMax Then
                    WorkerMsg(tMsgID.Err, "Extrapolation not allowed: R85 torque exceeds Fuel Map max torque at " & rl.TargetRPM & " [1/min] by more than defined limit!")
                    Return False
                End If
    ...
    Public Function WriteMap(ByVal path As String) As Boolean
        ...
        file.WriteLine("rpm [1/min],torque [Nm],FC [g/h]")
    ...
    Public Function WriteFLD(ByVal path As String) As Boolean
        ...
        file.WriteLine("rpm [1/min],full load torque [Nm],motoring torque [Nm]")
        ...
        For i = 0 To R85.iDim
            ...
            TqMaxMap = TqMax(nU)
            If Tq > TqMaxMap Then
                WorkerMsg(tMsgID.Warn, "   Actual engine R49 Full Load torque exceeds Fuel Map max torque at " & Math.Round(nU, 2) & " [1/min] by " & Math.Round(Tq - TqMaxMap, 2) & " [Nm] !")
                WorkerMsg(tMsgID.Warn, "     >>> Maximum torque at " & Math.Round(nU, 2) & " [1/min] will be limited to " & Math.Round(TqMaxMap, 2) & " [Nm] in full load output file!")
                Tq = TqMaxMap
            End If
            file.WriteLine(nU, Tq, DragTq)
        Next
    ...
    Public Function fFCdelaunay_Intp(ByVal nU As Single, ByVal Tq As Single) As Single
        ...
        If FuelMap.ExtrapolError Then
            WorkerMsg(tMsgID.Err, "Cannot extrapolate FC map! n= " & nU.ToString("0.00") & " [1/min], Tq= " & Tq.ToString("0.00") & " [Nm]")
            Return -10000
        Else
            Return val
        End If
...
lbInt:
        'Interpolation
        Return (nU - rpm0) * (RPMlists(rpm).MapPoints.Last.Tq - RPMlists(rpm0).MapPoints.Last.Tq) / (rpm - rpm0) + RPMlists(rpm0).MapPoints.Last.Tq
    End Function
...
lbInt:
        'Interpolation
        Return (nU - rpm0) * (RPMlists(rpm).MapPoints.First.Tq - RPMlists(rpm0).MapPoints.First.Tq) / (rpm - rpm0) + RPMlists(rpm0).MapPoints.First.Tq
    End Function
