...
Public Class cMAP0
    ...
    Private TqList As New List(Of Double)

    Private FuelMap As cDelaunayMap

    Private LnU_out As New List(Of Double)
    Private LTq_out As New List(Of Double)
    Private LTqMot_out As New List(Of Double)
    Private LnU_outTemp As New List(Of Double)
    Private LTq_outTemp As New List(Of Double)
                ...
                Else
                    ...
                    If StringForSplit.Contains(".") Then
                        ...
                        If StringsAfterSplit(1).Length <> 2 Then
                            WorkerMsg(tMsgID.Err, "Value needs to have exactly 2 digits after the decimal point: " & StringForSplit & " [1/min]! Line number: " & iMapDim + 2 & " (" & FilePath & ")")
                            GoTo lbEr
                        End If
                    Else
                        WorkerMsg(tMsgID.Err, "Value does not contain a valid decimal separator: " & StringForSplit & " [1/min]! Line number: " & iMapDim + 2 & " (" & FilePath & ")")
                        GoTo lbEr
                    End If
                    ...
                    If StringForSplit.Contains(".") Then
                        StringsAfterSplit = StringForSplit.Split(New String() {"."c}, StringSplitOptions.RemoveEmptyEntries)
                        If StringsAfterSplit(1).Length <> 2 Then
                            WorkerMsg(tMsgID.Err, "Value needs to have exactly 2 digits after the decimal point: " & StringForSplit & " [Nm]! Line number: " & iMapDim + 2 & " (" & FilePath & ")")
                            GoTo lbEr
                        End If
                    Else
                        WorkerMsg(tMsgID.Err, "Value does not contain a valid decimal separator: " & StringForSplit & " [Nm]! Line number: " & iMapDim + 2 & " (" & FilePath & ")")
                        GoTo lbEr
                    End If
                    ...
                    If StringForSplit.Contains(".") Then
                        StringsAfterSplit = StringForSplit.Split(New String() {"."c}, StringSplitOptions.RemoveEmptyEntries)
                        If StringsAfterSplit(1).Length <> 2 Then
                            WorkerMsg(tMsgID.Err, "Value needs to have exactly 2 digits after the decimal point: " & StringForSplit & " [g/h]! Line number: " & iMapDim + 2 & " (" & FilePath & ")")
                            GoTo lbEr
                        End If
                    Else
                        WorkerMsg(tMsgID.Err, "Value does not contain a valid decimal separator: " & StringForSplit & " [g/h]! Line number: " & iMapDim + 2 & " (" & FilePath & ")")
                        GoTo lbEr
                    End If
...
lbEr:
    ...
    Public Function Init() As Boolean
        ...
        If IntervalCount <> IntervalTarget(0) Then
            WorkerMsg(tMsgID.Err, "Incorrect rpm intervals between n_idle and n_A (" & IntervalCount & " intervals, but should be " & IntervalTarget(0) & ")!")
            WorkerMsg(tMsgID.Err, "  >>> Check also tolerances for torque points at zero torque!")
            Return False
        End If
        ...
        If IntervalCount <> IntervalTarget(1) Then
            WorkerMsg(tMsgID.Err, "Incorrect rpm intervals between n_B and n_95h (" & IntervalCount & " intervals, but should be " & IntervalTarget(1) & ")!")
            WorkerMsg(tMsgID.Err, "  >>> Check also tolerances for torque points at zero torque!")
            Return False
        End If
    ...
    Public Function AddDrag() As Boolean
        ...
        Dim lTargetRpms As List(Of Integer) = RPMlists.Keys.ToList()
        Dim i As Integer
        ...
        For Each rl In RPMlists.Values
            ...
            If IndexTargetRpms > 1 AndAlso IndexTargetRpms < (lTargetRpms.Count - 2) Then
                ...
                For i = 0 To Motoring.iDim
                    If Motoring.LnU(i) >= (lTargetRpms(IndexTargetRpms) - dnUleft_forLoop / 2) AndAlso Motoring.LnU(i) < (lTargetRpms(IndexTargetRpms) + dnUright_forLoop / 2) Then
        ...
        'add additional point at (motoring torque -100Nm) with zero FC
        'rl.MapPoints.Insert(0, New cMapPoint(rl.TargetRPM, Motoring.Tq(lTargetRpms(1)) - 100, 0))
        ...
        'add additional point at (motoring torque -100Nm) with zero FC
        'rl.MapPoints.Insert(0, New cMapPoint(rl.TargetRPM, Motoring.Tq(lTargetRpms(lTargetRpms.Count - 2)) - 100, 0))
        ...
        report.Add(New XElement(rootNamespace + "VectoInputDeclaration",
                                ...
                                New XElement(rootNamespace + "Engine",
                                             New XElement(tns + "Data",
                                                          ...
                                                          New XElement(tns + "WHTCMotorway", job.WHTCmotorwayFactor.ToString("f4")),
...
lbInt:
    ...
    Private Function TqMax_FLC_cMAP(ByVal nU As Double) As Double
        Dim i As Int32
...
lbInt:
        'Interpolation
        Return (nU - LnU_outTemp(i - 1)) * (LTq_outTemp(i) - LTq_outTemp(i - 1)) / (LnU_outTemp(i) - LnU_outTemp(i - 1)) + LTq_outTemp(i - 1)
    End Function
